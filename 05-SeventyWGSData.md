...
Things went nice today. Did not spend a lot of try runs for the statistics code. I could run the statistics pipeline through all the genes to generate tons of figures! Also, there is a change for a PCI analysis by using the genetic distance (may need transformation since the closed related species have smaller number) from all reference species. 