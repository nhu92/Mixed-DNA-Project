...
def find_local_maxima(x, y, z):
    """
    Find local maxima in the actual data points.
    """
    ...
    for i in range(len(z)):
        ...
        if all(z[i] >= z[j] for j in indices):
...
def main(input_file, output_file):
    ...
    positive_local_maxima_points = [(x, y) for x, y in local_maxima_points if z[np.where((x == x) & (y == y))[0][0]] >= 0]
    ...
    for x_peak, y_peak in positive_local_maxima_points:
